...
def create_new_tables():
    ...
    # No longer need zones table
    # Create tiles table with map_id instead of zone_id
    cursor.execute(
        """
...
def populate_tiles(conn, block_pos_to_image_id):
    ...
    # Create a mapping of map name to map_id and is_overworld flag
    map_name_to_map_info = {}
    for map_id, map_name, is_overworld in maps_data:
        # Store the map info
        map_name_to_map_info[map_name] = (map_id, is_overworld)

        # Handle special cases for routes
        if map_name.startswith("ROUTE_"):
            route_num = map_name[6:]  # Extract route number
            route_alt_name = f"ROUTE{route_num}"
            map_name_to_map_info[route_alt_name] = (map_id, is_overworld)
    ...
    cursor.execute(
        """
    SELECT m.id, m.name, m.width, m.height, m.tileset_id, m.is_overworld
    FROM maps m
    """
    )
    ...
    for i, (
        ...
        # Get the map_id and is_overworld flag for this map
        map_info = map_name_to_map_info.get(map_name)

        if map_info:
            map_id, is_overworld = map_info
        else:
            # If not found in our mapping, use the one from the database join if available
            if is_overworld is not None:
                is_overworld = is_overworld
            else:
                # Try a more flexible match
                map_name_simplified = map_name.replace("_", "").lower()
                for z_name, z_info in map_name_to_map_info.items():
                    if z_name.replace("_", "").lower() == map_name_simplified:
                        map_id, is_overworld = z_info
                        break
                else:
                    map_id = None
                    is_overworld = 0
    ...
    print(f"\nSkipped {skipped_maps} maps due to missing map_id mapping:")
...
def main():
    ...
    print("\nSummary:")
    print(f"- Created {map_count} maps")
